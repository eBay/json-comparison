...
public class IgnoredModifiedValuesFilterTest {

  private IgnoredModifiedValuesFilter ignoredModifiedValuesFilter;
  ...
  @Before
  public void setUp() {
    when(valueConfig.getRegexPaths()).thenReturn(Lists.newArrayList("parent\\.path\\.2", "parent\\.path2\\.a"));
    ignoredModifiedValuesFilter = new IgnoredModifiedValuesFilter(valueConfig);
  }

  @Test
  public void shouldFilterModifiedValuesAccordingToGivePattern_verifyResults() {
    ...
    JsonComparatorResult actualResult = ignoredModifiedValuesFilter.filter(jsonComparatorResult);
    assertThat(actualResult.getModifiedFields().size(), equalTo(0));
    assertThat(actualResult.getMissingFields().size(), equalTo(1));
    assertThat(actualResult.getNewFields().size(), equalTo(1));
  }

  @Test
  public void shouldNotFilterModifiedValuesAccordingToGivePattern_verifyResults() {
